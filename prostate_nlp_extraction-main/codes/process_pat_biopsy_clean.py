...
#		df_pathology = df_pathology.loc[df_pathology.Report_Description == 'Surgical Pathology']
		# df_pathology = pd.read_csv('../data/df_pathology_sampled_edited.csv')
		# breakpoint()
		...
		#
		# df_pathology_with_biopsy = pd.read_csv('../data/df_pathology_sampled_edited.csv')
		df_pathology_with_biopsy = pd.read_csv('../data/sample_biopsy_reports.csv')
		# df_pathology_with_biopsy = df_pathology_with_biopsy.iloc[[0]]
			...
			for idx, val in enumerate(text_oi_list):
				...
				if 'gleason' in val and 'm.d.,' not in text_oi_list[idx - 2 : idx + 2]:
					...
					if 'of' in raw_gleason_cores_context:
						for idx_core_word, core_word in enumerate(raw_gleason_cores_context):
							if core_word == 'of':
								...
								### captures all the different ways to say X of X
					...
					if slash_counter == 1 and plus_sign_counter == 0 and dash_counter == 0:
						only_one_slash = 1
# checks to see if any numbers are written as the number word; changes to the number
					for idx_char, char in enumerate(raw_gleason_text):
						if char in desc_num_words_list:
							raw_gleason_text[idx_char] = desc_num_to_num_dic[char]

					# indicator if primary and secondary grades not provided, just overall grade out of 10. used for
					# determining grade group
					...
					for idx_char, char in enumerate(raw_gleason_text):
						...
						if char == '(':
							left_paren_idx = idx_char
							raw_gleason_equation = raw_gleason_text[idx_char:idx_char + 10]
							for sub_idx_char,sub_char in enumerate(raw_gleason_equation):
								if sub_char == ')':
									gleason_in_parens = raw_gleason_equation[0:sub_idx_char]
									digits_in_paren_statement = ''.join(c for c in gleason_in_parens if c.isdigit())
									if len(digits_in_paren_statement) == 2:
										primary_grade = digits_in_paren_statement[0]
										found_primary_grade = 1
										secondary_grade = digits_in_paren_statement[1]
										found_secondary_grade = 1
							break
						...
						if char == "and":
							raw_gleason_equation = raw_gleason_text[idx_char-10:idx_char+10]
							# exclude "of 5" and "/5"
							raw_gleason_equation = re.sub('of 5|/5', '', raw_gleason_equation)
							digits_in_and_statement = ''.join(c for c in raw_gleason_equation if c.isdigit())
							if len(digits_in_and_statement) == 2:
								primary_grade = digits_in_and_statement[0]
								found_primary_grade = 1
								secondary_grade = digits_in_and_statement[1]
								found_secondary_grade = 1
							...
							break
						if char == "of" and raw_gleason_text[idx_char+1] == 10:
							if raw_gleason_text[idx_char-1].isdigit():
								total_score = raw_gleason_text[idx_char-1]
