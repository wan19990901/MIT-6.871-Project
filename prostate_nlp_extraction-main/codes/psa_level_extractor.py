...
def post_process_psa_result(df_psa_result):
	...
	for empi, psa_val, date, comment in zip(df_psa_result.index, df_psa_result.psa_val.values, df_psa_result.date.values, df_psa_result.comment.values):
		empi_true = empi.split('_')[0]
		if empi_true in empi_to_psa_trajectory_date_dic.keys():
			if not pd.isnull(comment):
				empi_to_psa_trajectory_date_dic[empi_true] = empi_to_psa_trajectory_date_dic[empi_true] | {(psa_val, date, '<')}
			else:
				empi_to_psa_trajectory_date_dic[empi_true] = empi_to_psa_trajectory_date_dic[empi_true] | {(psa_val, date, '')}
		else:
			if not pd.isnull(comment):
	...
	for empi, psa_vals in empi_to_psa_trajectory_date_dic.items():
		idx_counter = 0;
		df_psa_result_per_empi = pd.DataFrame([], index = np.arange(len(psa_vals)),columns = ['empi', 'psa_val', 'date', 'comment'])
...
def get_psa_vals(df_prg_oi):
	df_result = pd.DataFrame([], columns = ['psa_val', 'date', 'comment']); empi_psa_list = set(); empi_idx_counter = 0
	empi_prev = ''
	for empi, report in tqdm(zip(df_prg_oi.index, df_prg_oi.Report_Text.values), total = len(df_prg_oi), desc = 'Getting PSA vals...'):
	#     empi_idx_counter = 
		if empi != empi_prev:
			empi_idx_counter = 0
		...
		if 'psa' in report_lower_case:
			...
			for idx, word in enumerate(report_split):
				ineqaul_ind = 0
				if 'psa' in word and idx + 2 < len(report_split):
					potential_date_val = report_split[idx + 2]
					...
					if potential_date_val.count('/') == 2:
						...
						if '<' in potential_psa_val:
							...
							try:
								df_result.at[str(empi) + '_' + str(empi_idx_counter), 'psa_val'] = float(potential_psa_val_num)
								ineqaul_ind = 1 
							except:
								# empi_idx_counter += 1
								continue
							...
							try:
								df_result.at[str(empi) + '_' + str(empi_idx_counter), 'psa_val'] = float(potential_psa_val)
							except:
								# empi_idx_counter += 1
								continue
...
def main():
	...
	print('Number of unique patients (not in PSA reports && in Biopsy reports && in Progress notes) : ', len(empi_in_pat_not_in_lab_in_prg))
	empi_in_psa_biopsy = set(df_prg_total.index) & set(df_pathology_biopsy_final.index)
	empi_in_psa_biopsy_progress = set(df_prg_total.index) & set(df_pathology_biopsy_final.index) & set(df_labs_processed.index)
	print('Number of unique patients (in Biopsy reports && in Progress notes) : ', len(empi_in_psa_biopsy))
	print('Number of unique patients (in PSA reports && in Biopsy reports && in Progress notes) : ', len(empi_in_psa_biopsy_progress))
